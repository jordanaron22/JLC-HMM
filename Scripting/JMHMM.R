...
incl_act <- 1
#bool, set to T to include light
incl_light <- 0
...
#0 for standard, 1 to load data for hot start
subset_data <- as.numeric(commandArgs(TRUE)[8])
print(paste("Subset",subset_data))

#int (0-2), sets simulation size
sim_size <- as.numeric(commandArgs(TRUE)[9])
print(paste("Sim Size",sim_size))
...
if(is.na(randomize_init)){randomize_init <-0}
if(is.na(load_data)){load_data <- 1}
if(is.na(leave_out)){leave_out <- 0}
if(is.na(subset_data)){subset_data <- 0}
...
if (subset_data){model_name <- paste0(model_name,"Subset",subset_data)}
if (randomize_init){model_name <- paste0(model_name,"RandInit")}
if (load_data){model_name <- paste0(model_name,"LoadIn")}

model_name <- paste0(model_name,"Mix",mix_num,"Seed",sim_num,"len",period_len,".rda")
print(model_name)
################## Functions ##################
#Reads in rcpp file
...
#calculates integrated brier score
...
if (misspecification){
  ...
  params_tran_week_true <- matrix(rep(c(0,0,0,0,0,0),mix_num),ncol = 6,byrow = T) 
  params_tran_week_true[,1] <- seq(-2.9,-2.3,length.out = mix_num)
  params_tran_week_true[,2] <- seq(1.6,1.2,length.out = mix_num)
  params_tran_week_true[,3] <- seq(.3,.8,length.out = mix_num)
  params_tran_week_true[,4] <- seq(-1.8,-2.2,length.out = mix_num)
  params_tran_week_true[,5] <- seq(-1.6,-1.2,length.out = mix_num)
  params_tran_week_true[,6] <- seq(-.5,-.7,length.out = mix_num)
  
  
  params_tran_weekend_true <- matrix(rep(c(0,0,0,0,0,0),mix_num),ncol = 6,byrow = T) 
  params_tran_weekend_true[,1] <- seq(-3.1,-2.5,length.out = mix_num)
  params_tran_weekend_true[,2] <- seq(1,.8,length.out = mix_num)
  params_tran_weekend_true[,3] <- seq(.7,.9,length.out = mix_num)
  params_tran_weekend_true[,4] <- seq(-1.8,-2.4,length.out = mix_num)
  params_tran_weekend_true[,5] <- seq(-1.3,-.9,length.out = mix_num)
  params_tran_weekend_true[,6] <- seq(-.9,-1.1,length.out = mix_num)
  ...
  emit_act_weekend_true[1,1,] <- seq(5,6,length.out = mix_num)
  emit_act_weekend_true[1,2,] <- seq(3,4,length.out = mix_num)
  emit_act_weekend_true[2,1,] <- seq(2,2,length.out = mix_num)
  emit_act_weekend_true[2,2,] <- seq(3,2,length.out = mix_num)
  ...
  emit_light_week_true[1,1,] <- seq(-2,-1,length.out = mix_num)
  emit_light_week_true[1,2,] <- seq(8,6,length.out = mix_num)
  emit_light_week_true[2,1,] <- seq(-19,-19,length.out = mix_num)
  emit_light_week_true[2,2,] <- seq(13,15,length.out = mix_num)
  
  emit_light_weekend_true <- array(NA, c(2,2,mix_num))
  emit_light_weekend_true[1,1,] <- seq(-3,-2,length.out = mix_num)
  emit_light_weekend_true[1,2,] <- seq(9,7,length.out = mix_num)
  emit_light_weekend_true[2,1,] <- seq(-21,-21,length.out = mix_num)
  emit_light_weekend_true[2,2,] <- seq(15,18,length.out = mix_num)
  ...
  corr_mat_true <- array(NA, c(mix_num,2,2))
  corr_mat_true[,1,1] <- seq(.1,.2,length.out = mix_num)
  corr_mat_true[,2,1] <- seq(.2,.3,length.out = mix_num)
  corr_mat_true[,1,2] <- seq(.3,.4,length.out = mix_num)
  corr_mat_true[,2,2] <- seq(.4,.5,length.out = mix_num)
...
if (real_data) {
  ...
  if(subset_data == 1){
    subset_ind <- which(id$age >= 75)
  } else if (subset_data == 2){
    subset_ind <- which(id$race == 0)
  } else if (subset_data == 3){
    subset_ind <- which(id$race == 3)
  } else if (subset_data == 4){
    subset_ind <- which(id$poverty <= 1)
  } else if (subset_data == 5){
    subset_ind <- which(id$poverty >= 1)
  } else if (subset_data == 6){
    subset_ind <- which(id$BMI >= 35)
  }
...
if (leave_out){
  ...
  post_decode_collapsed_true <- to_save[[2]][[18]][,leave_out_inds]
  ...
  model_name_loadin <- paste0(model_name_loadin,"Mix",mix_num,"Seed",sim_num,"len96.rda")
...
while((abs(like_diff) > stop_crit | iter_count < 5) & !run_only_surv){
  ...
  if ((abs(like_diff) < stop_crit*10) & !relabel_reset & !bootstrap & !leave_out & real_data){
    relabel_reset <- TRUE
    relabel_bool <- 0
    print("Relabelling")
...
if (leave_out){
  ...
  leave_out_to_save <- list(leave_out_inds,conf_mat_list,cindex_new_list,ibs_new_list,senspec_list,ibs2_new_list,senspec_mix_list)
} else {
...
diagnostics <- list(cindex,ibs,tab,ibs2)
#save everything
bic <- CalcBIC(new_likelihood,mix_num,act,light)
