    ...
    for RE_num in 5; do
        for real_data in 0; do
            for bootstrap in 0; do
                for randomize_init in 0 0.5; do
                    for leave_out in 0; do
                        for load_data in 0; do
                            for subset_data in 0; do
                                for sim_size in 0 1 2; do
                                    sbatch --time="${TimeArray[$RE_num]}":00:00 --mem="${MemArray[$RE_num]}"gb SubLoopJMHMM4.sh $RE_num $surv $real_data $bootstrap $randomize_init $leave_out $load_data $subset_data
                                done
                            done
                        done
                    done
