...
model_name <- paste0(model_name,"Mix",mix_num,"Seed",".rda")
print(model_name)
################## Functions ##################
...
for (sim_num in 1:65){
  ...
  if (misspecification){
    ...
    emit_act_weekend_true[2,1,] <- seq(9,0,length.out = mix_num)
  ...
  if (load_data){
    ...
    if (length(to_save[[2]]) > 8){
      ...
      # NEED TO CHANGE THIS TOO
      # FIRST 13 SHOULD BE 12
      #Non-tobit is defunct as of now
      lambda_act_mat_true <- to_save[[2]][[13]]
      lambda_light_mat_true <- to_save[[2]][[13]]
  ...
  if (real_data) {
    ...
    if (add_noise){
      act <- act + rnorm(prod(dim(act)),0,1)
      light <- light + rnorm(prod(dim(light)),0,1)
    }
  ...
  for (i in 1:length(surv_coef)){
    surv_coef[[i]] <-surv_coef_true[[i]]  +  runif(length(surv_coef_true[[i]]),-runif_tol/10,runif_tol/10)
    if (length(surv_coef[[i]]) != 1){surv_coef[[i]][1] <- 0} 
  }
  surv_coef[[1]] <- surv_coef_true[[1]] + runif(1,-runif_tol/100,runif_tol/100)
  ...
  tlogmims_mat <- rbind(wave_data_G[[4]],wave_data_H[[4]])
  tlogmims_mat <- tlogmims_mat[seqn_com_id,][-leave_out_inds,]
  surv_covar[length(surv_covar)+1] <- list(tlogmims_mat[,1])
  # surv_covar[length(surv_covar)+1] <- list(tlogmims_mat[,4])
  
  surv_data <- data.frame(time = surv_time,
                          status = surv_event,
                          age = surv_covar[[1]],
                          tlogmims = surv_covar[[15]])#,
                          # RAL = surv_covar[[16]])
  
  surv_data <- cbind(surv_data,combined_covar_mat)
  surv_data <- surv_data[,c(1:3,5:17,4)]
  # surv_data <- surv_data[,c(1:3,6:18,4,5)]
  
  fit <- coxph(Surv(time, status) ~ . , data = surv_data)
  beta_surv_coef_new <- fit$coefficients
  se <- sqrt(diag(fit$var))
  
  surv_coef_len <- c(surv_coef_len,1)
  # surv_coef_len <- c(surv_coef_len,1,1)
  surv_coef <- OutofBetaSurvCoef(beta_surv_coef_new,surv_coef_len)[[2]]
  surv_coef[[15]] <- beta_surv_coef_new[31]
  # surv_coef[[16]] <- beta_surv_coef_new[32]
  
  surv_covar[[15]] <- matrix(surv_covar[[15]],ncol = 1)
  surv_coef[[15]] <- matrix(surv_coef[[15]],ncol = 1)
  # surv_covar[[16]] <- matrix(surv_covar[[16]],ncol = 1)
  # surv_coef[[16]] <- matrix(surv_coef[[16]],ncol = 1)
  ...
  tlogmims_mat_new <- rbind(wave_data_G[[4]],wave_data_H[[4]])
  tlogmims_mat_new <- tlogmims_mat_new[seqn_com_id,][leave_out_inds,]
  new_surv_covar[length(new_surv_covar)+1] <- list(tlogmims_mat_new[,1])
  # new_surv_covar[length(new_surv_covar)+1] <- list(tlogmims_mat_new[,4])
  ...
  re_prob_new <- matrix(rep(1,length(leave_out_inds),ncol = 1))
  ...
  diagnostics <- list(cindex,ibs)
  
  #"~/JM/Routputs"
  setwd("Leroux")
  save(diagnostics,file = paste0("LerouxCV",sim_num,".rda"))
  setwd("..")
}

ManualBrier <- function(curr_time){
  return(brier(fit, times=c(curr_time))$brier)
}
integrate(ManualBrier,0,max(surv_time_new), rel.tol = .01)$value/max(surv_time_new)
