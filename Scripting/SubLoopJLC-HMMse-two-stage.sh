...
h2_eps=${16:-1e-5}
retain_score_matrices=${17:-false}

date
path="/projects/standard/mfiecas/aron0064/JLC-HMM"
cd "$path/Routputs" || exit 1
start_time=$(date +%s)
module load R/4.4.0-openblas-rocky8
echo "JLC-HMMse two-stage Murphy-Topel epsilon: $h2_eps"
echo "Retain S1/S2 score matrices: $retain_score_matrices"
Rscript "$path/Rcode/JLC-HMMse-two-stage.R" "$fit_mix_num" "$model_type" "$data_source" "$run_bootstrap" "$init_jitter_scale" "$run_leave_one_out_cv" "$use_hot_start" "$simulation_days" "$num_people" "$true_mix_num" "$save_reduced_output" "$class_selection_run" "$emission_overlap" "$time_limit_hours" "$memory_limit_gb" "$h2_eps" "$retain_score_matrices"
finish_time=$(date +%s)
