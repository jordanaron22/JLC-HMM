...
if (leave_out){
  ...
  for (leave_out_type in 1:6){
    ...
    if (leave_out_type != 3){
      ...
      for (lohi_med in 1:3){
        if (lohi_med == 1){
          valid_inds <- as.vector(new_act_working < quantile(act,1/2,na.rm = T))
          pdc_colnames <-  c("Below-Med Pred Wake","Pred Sleep")
        } else if (lohi_med == 2) {
          valid_inds <- as.vector(new_act_working > quantile(act,1/2,na.rm = T))
          pdc_colnames <-  c("Above-Med Pred Wake","Pred Sleep")
        } else {
          valid_inds <- as.vector(new_act_working > -Inf)
          pdc_colnames <-  c("Pred Wake","Pred Sleep")
        }
        
        valid_inds[is.na(valid_inds)] <- F
        
        coda <- c(T,F)
        
        pdc_tab <- table(c(post_decode_collapsed_vec[valid_inds],coda),c(post_decode_collapsed_true_vec[valid_inds],coda))
        ...
        # #specificity
        # sens <- diag(pdc_tab/rowSums( pdc_tab))
        # #sensitivity
        # spec <- diag(t(t(pdc_tab)/colSums( pdc_tab)))
        # 
        # sens_spec_df_working <- data.frame(prob = c(sens,spec),
        #                                    lohi = lohi_med,
        #                                    type = c("Sensitivity","Sensitivity","Specificity","Specificity"),
        #                                    state = c("wake","sleep"))
        # 
        # sens_spec_df <- rbind(sens_spec_df,sens_spec_df_working)
    }
  ...
  leave_out_to_save <- list(leave_out_inds,conf_mat_list,cindex_new_list,ibs_new_list,senspec_list,ibs2_new_list)
} else {
