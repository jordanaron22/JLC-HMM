...
for (i in seq_len(nrow(dat))){
  ...
  coverage_rows[[length(coverage_rows) + 1]] <- cbind(run_data,beta_data)
}

coverage_coef_data <- do.call(rbind,coverage_rows)
row.names(coverage_coef_data) <- NULL

large_coverage_beta_se <- coverage_coef_data[
  is.finite(coverage_coef_data$beta_se) &
    coverage_coef_data$beta_se > 100 |
    is.finite(coverage_coef_data$reference_beta_se) &
    coverage_coef_data$reference_beta_se > 100,
]

coverage_coef_data <- coverage_coef_data[
  !(is.finite(coverage_coef_data$beta_se) &
      coverage_coef_data$beta_se > 100 |
      is.finite(coverage_coef_data$reference_beta_se) &
      coverage_coef_data$reference_beta_se > 100),
...
coverage_coef_data$contrast_se <- sqrt(
  ifelse(is.na(coverage_coef_data$beta_se),0,
         coverage_coef_data$beta_se^2) +
    ifelse(is.na(coverage_coef_data$reference_beta_se),0,
           coverage_coef_data$reference_beta_se^2)
)
coverage_coef_data$lower <- coverage_coef_data$fit_beta -
  1.96 * coverage_coef_data$contrast_se
coverage_coef_data$upper <- coverage_coef_data$fit_beta +
  1.96 * coverage_coef_data$contrast_se
coverage_coef_data$covered <- coverage_coef_data$true_beta >=
  coverage_coef_data$lower &
  coverage_coef_data$true_beta <= coverage_coef_data$upper

coverage_data <- aggregate(
  covered ~ true_mix_num + fit_mix_num + model_type +
    simulation_days + num_people + emission_overlap,
  data = coverage_coef_data,
  FUN = mean
)
names(coverage_data)[names(coverage_data) == "covered"] <- "coverage"

coverage_n_seeds <- aggregate(
  sim_num ~ true_mix_num + fit_mix_num + model_type +
    simulation_days + num_people + emission_overlap,
  data = coverage_coef_data,
  FUN = function(x) length(unique(x))
)
names(coverage_n_seeds)[names(coverage_n_seeds) == "sim_num"] <- "n_seeds"

coverage_n_coef <- aggregate(
  covered ~ true_mix_num + fit_mix_num + model_type +
    simulation_days + num_people + emission_overlap,
  data = coverage_coef_data,
  FUN = length
)
names(coverage_n_coef)[names(coverage_n_coef) == "covered"] <- "n"

coverage_data <- merge(coverage_data,coverage_n_seeds,by = scenario_cols)
coverage_data <- merge(coverage_data,coverage_n_coef,by = scenario_cols)

coverage_data$days <- coverage_data$simulation_days
coverage_data$days_label <- factor(
  paste0(coverage_data$days," Day",
         ifelse(coverage_data$days == 1,"","s")),
  levels = levels(plot_data$days_label)
)
coverage_data$model_type_label <- factor(
  ifelse(coverage_data$model_type == "joint","JLCM","Two-Stage"),
  levels = c("JLCM","Two-Stage")
)
coverage_data$overlap_label <- factor(
  paste0("Overlap: ",title_case(coverage_data$emission_overlap)),
  levels = levels(plot_data$overlap_label)
)
coverage_data$people_label <- factor(
  paste0("n = ",format(coverage_data$num_people,big.mark = ",",
                       scientific = FALSE,trim = TRUE)),
  levels = levels(plot_data$people_label)
)

coverage_data <- coverage_data[,c("true_mix_num","fit_mix_num",
                                  "model_type","days","num_people",
                                  "emission_overlap","coverage",
                                  "n_seeds","n","days_label",
                                  "model_type_label","overlap_label",
                                  "people_label")]
row.names(coverage_data) <- NULL
...
if (requireNamespace("ggplot2",quietly = TRUE)){
  ...
  p_rmse <- ggplot2::ggplot(
    plot_data,
    ggplot2::aes(x = factor(days), y = rmse, fill = factor(model_type))
  ) +
    ggplot2::geom_col(position = ggplot2::position_dodge()) +
    ggplot2::facet_grid(rows = ggplot2::vars(emission_overlap),
                        scales = "free_y")

  ggplot2::ggsave("survival_rmse.png", plot = p_rmse,
                  path = "Output/Plots",width = 10, height = 6, dpi = 300)

  p_coverage <- ggplot2::ggplot(
    coverage_data,
    ggplot2::aes(x = factor(days), y = coverage,
  ...
  ggplot2::ggsave("survival_beta_coverage.png", plot = p_coverage,
                  path = "Output/Plots",width = 10, height = 6, dpi = 300)
} else {
  p_rmse <- NULL
  p_coverage <- NULL
  message("ggplot2 is not installed, so plot_data and coverage_data were created without plots.")
}
